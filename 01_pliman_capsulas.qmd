...
img <- image_import("a1426e78-6bc2-4ce4-9d58-0777384c066d.jpg")
res <- 
  analyze_objects(
    img,
    resize = 50,
...
df <- 
  ...
  as_factor(filter:resolucao) %>% 
  mutate(filter = paste0("filter = ", filter),
         opening = paste0("opening = ", opening))

ccc_lin <- function(data, real, predito) {
  # extrai os valores para um vetor
  if(is.grouped_df(data)){
    data %>% 
      group_modify(~ccc_lin(.x, {{real}}, {{predito}})) %>% 
      ungroup()
  } else{
    x <- data |> select({{real}}) |> pull()
    y <- data |> select({{predito}}) |> pull()
    ...
    # retorna
    return(data.frame(r = r, rmse = rmse, ccc = ccc))
  }
